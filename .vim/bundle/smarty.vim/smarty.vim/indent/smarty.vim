...
" General:
" File:       smarty.vim (Vimscript #2075)
...
" Vim Version:     Vim7
" Description:
"   Altered version of the distributed html indent script, also indents
"   Smarty tags. Based on HTML indent v0.9 by Andy Wokula
"
" Credits:
"       indent/html.vim (2013 Jun 12) from A. Wokula
"  indent/html.vim (2006 Jun 05) from J. Zellner
"  indent/css.vim (2006 Dec 20) from N. Weibull
"
" History:
" 2014 Aug 12   (v0.1) added smarty.net support
...
" Smarty tags that affect indentation
call s:AddITags([ 'capture', 'if', 'ldelim', 'rdelim', 'literal', 'php',
      \ 'section', 'sectionelse', 'foreach', 'foreachelse', 'strip', 'cycle',
      \ 'textformat', 'else', 'elseif'])

" I think the head tag should be indented
call s:AddITags(['head'])
...
func! s:CountITags(...)
  ...
  if a:0==0
    ...
    let tmpline = substitute(s:curline, '{\zs\/\=\w\+\>\|<\zs\/\=\w\+\>\|<!--\|-->', '\=s:CheckTag(submatch(0))', 'g')
    if s:block == 3
    ...
    let tmpline = substitute(s:altline, '{\zs\/\=\w\+\>\|<\zs\/\=\w\+\>\|<!--\|-->', '\=s:CheckTag(submatch(0))', 'g')
    let s:countonly = 0
  endif
endfunc
...
func! s:FreshState(lnum)
  ...
  elseif stopline == state.lnum
    ...
    " blocktag == "/..."
    let swendtag = match(tagline, '^\s*</\|^\s*{/') >= 0
    if !swendtag
  ...
  let swendtag = match(s:altline, '^\s*</\|^\s*{/') >= 0
  if !swendtag
...
func! HtmlIndent()
  ...
  " does the line start with an else statement
  let elseline = match(s:curline, '^\s*{else\|^\s*{sectionelse\|^\s*{foreachelse') >= 0
  ...
  if b:indent.block >= 2
    ...
    if swendtag
      ...
      let s:newstate.baseindent = indent + s:nextrel * indentunit
    elseif elseline
      let indent = b:indent.baseindent - indentunit
    else
...
" check user settings (first time), clear cpo, Modeline:
